...
export const signup = (first_name, last_name, email, password, re_password) => async dispatch => {
    ...
    //YA CONFIGURADO, REALIZAMOS EL LLAMADO.
    try {
        //ESTA VARIABLE ESTA CONFIGURADA EN .ENV
        ...
        if (res.status === 201) {
            ...
            dispatch(setAlert('Error en crear la cuenta, intente nuevamente', 'red'))
        }
        //SI VA TODO BIEN O MAL, HAY QUE TERMINAR DE CARGAR
        dispatch({
...
export const activate = (uid, token) => async dispatch => {
    ...
    try {
        ...
        if (res.status === 204) {
            ...
            dispatch(setAlert('Activacion correcta.','green'));
        } else {
            dispatch({
                type: ACTIVATION_FAIL
            });
            dispatch(setAlert('Activacion erronea.', 'red'));
        }
        dispatch({
            type: REMOVE_AUTH_LOADING
        });
        ...
        dispatch(setAlert('El servidor no ha activado la cuenta.','red'));
